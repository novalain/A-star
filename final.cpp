...
int FindPath(const int nStartX, const int nStartY,
  ...
  while (!queue.empty()) {
    ...
    // Right
    {
      ...
      // Node neighbor = map[ComputeId(neighborX, node.y, nMapWidth)];

      // if (neighbor.x < nMapWidth && pMap[neighbor.id] && (!visited || node.g < neighbor.g) {
      //   neighbor.g = node.g + 1;
      //   neighbor.f = node.f 
      // }
    ...
    // Left
    {
      ...
      if (neighborX >= 0 && pMap[neighborId]) {
        ...
        if (currentDistNeighbor == -1 || (currentDist + 1 < currentDistNeighbor)) {
          Node neighbor = {neighborX, node.y, neighborId, 0};
          distances[neighbor.id] = currentDist + 1;
          int priority = distances[node.id] + 1 +
                         Manhattan(neighbor.x, neighbor.y, nTargetX, nTargetY);
          neighbor.f = priority;
          predecessors[neighbor.id] = node.id;
          queue.push(neighbor);
